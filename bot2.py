...
def func(message):
    ...
    if message.text == "Что ты можешь?":
        ...
        markup.add(ask)
        bot.send_message(message.from_user.id, "Часто мы не можем взять себя в руки и начать делать что-то полезное для себя каждый день," \
        ...
        bot.send_message(message.chat.id, text="На такую комманду я не запрограммирован.../start")
